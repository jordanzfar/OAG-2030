...
import { CheckCircle, AlertTriangle, Info, Clock, Bell, Search, Users, FileText, CreditCard, BookMarked as MarkAsUnread, Trash2 } from 'lucide-react'; // Añadido Trash2 para eliminar
...
const AdminNotificationsPage = () => {
    ...
    const { user, isLoading: userLoading } = useSupabaseAuth(); // Obtener usuario y estado de carga de autenticación
    const { fetchRecords, updateRecord, deleteRecord } = useSupabaseData(); // Obtener funciones de Supabase Data

    const [notifications, setNotifications] = useState([]);
    const [filteredNotifications, setFilteredNotifications] = useState([]);
    const [loading, setLoading] = useState(true); // Controla la carga inicial de datos de Supabase
    const [searchTerm, setSearchTerm] = useState('');
    const [filterType, setFilterType] = useState('all');
    const [filterStatus, setFilterStatus] = useState('all');

    useEffect(() => {
        // Solo cargar notificaciones si el usuario está disponible y no está cargando el estado de autenticación
        if (!userLoading && user) {
            loadNotifications();
        } else if (!userLoading && !user) {
            // Si no hay usuario autenticado (admin), no se cargan notificaciones
            setLoading(false);
            console.log("No hay usuario administrador autenticado para cargar notificaciones.");
        }
    ...
    const loadNotifications = async () => {
        ...
        if (!user) {
            console.log("No hay usuario para cargar notificaciones administrativas.");
            setLoading(false);
            return;
        }

        // Suponemos que las notificaciones de admin tienen un user_id que coincide con el admin actual
        // O podrías tener un campo 'is_admin_notification: true' y buscar todas ellas si son globales
        ...
        if (result.success) {
            ...
            console.log("Notificaciones de administrador cargadas:", result.data);
        } else {
            console.error("Error al cargar notificaciones de administrador:", result.error);
            toast({
                title: "Error al cargar notificaciones",
                description: result.error?.message || "Ocurrió un error al cargar las notificaciones administrativas.",
                variant: "destructive"
            });
            setNotifications([]); // Asegurarse de que no queden datos viejos
        }
    ...
    const markAllAsRead = async () => {
        ...
        // Actualiza el estado local primero para una respuesta rápida en la UI
        ...
        // Espera a que todas las actualizaciones se completen en Supabase
        const results = await Promise.all(updatePromises);

        // Opcional: Manejar errores individuales si alguna actualización falló
    ...
    const getNotificationIcon = (type) => {
        switch (type) {
            ...
            // Los tipos del cliente también podrían aparecer aquí si comparten la misma tabla
    ...
    const getNotificationTypeBadge = (type) => {
        switch (type) {
            case 'new_user':
                return <Badge variant="default">Nuevo Usuario</Badge>;
            case 'new_request':
                return <Badge variant="default">Nueva Solicitud</Badge>;
            case 'payment_pending':
                return <Badge variant="destructive">Pago Pendiente</Badge>;
            case 'document_review':
                return <Badge variant="secondary">Revisión Documento</Badge>;
            case 'system_alert':
                return <Badge variant="destructive">Alerta Sistema</Badge>;
            case 'verification_required':
                return <Badge variant="secondary">Verificación Requerida</Badge>;
            // Los tipos del cliente también podrían aparecer aquí
            case 'payment_required':
                return <Badge variant="destructive">Pago Requerido Cliente</Badge>;
            case 'document_missing':
                return <Badge variant="destructive">Documento Cliente Faltante</Badge>;
            case 'status_update':
                return <Badge variant="default">Actualización Cliente</Badge>;
            case 'info':
                return <Badge variant="secondary">Información Cliente</Badge>;
            default:
    ...
    // Muestra un spinner de carga mientras se obtienen los datos o se carga el usuario
    ...
    // Si no hay usuario autenticado después de la carga, o si el usuario no tiene permiso (aunque ProtectedRoute debería manejar esto)
    if (!user) {
        return (
            <div className="space-y-6">
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                        ...
                        <Select value={filterType} onValueChange={setFilterType}>
                            ...
                            <SelectContent>
                                ...
                                {/* Añade aquí los tipos de cliente si la misma tabla contendrá ambos tipos de notificaciones */}
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-start justify-between mb-2">
                                                    ...
                                                    <div className="flex items-center space-x-2">
                                                        ...
                                                        >
                                                            <Trash2 className="w-4 h-4 mr-1" /> {/* Icono de bote de basura */}
                                                            Eliminar
                                                        </Button>
                                                    </div>
                                                </div>
