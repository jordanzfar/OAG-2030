...
serve(async (req) => {
  ...
  try {
    console.log("Función 'cancel-deposit' invocada.");
    ...
    console.log(`Intentando cancelar Payment Intent: ${payment_intent_id}`);

    await stripe.paymentIntents.cancel(payment_intent_id)
    console.log("Payment Intent cancelado en Stripe.");
    ...
    return new Response(JSON.stringify({ message: "Depósito cancelado con éxito." }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    })
  } catch (error) {
    console.error("¡ERROR EN cancel-deposit!", error); // <-- LÍNEA CLAVE AÑADIDA
